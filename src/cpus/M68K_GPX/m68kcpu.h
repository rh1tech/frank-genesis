...
/* Read data immediately following the PC - use FETCH macros instead of memory_map */
#define m68k_read_immediate_16(address) ( ( (address) & 0x800000) ? FETCH16RAM((address)) : FETCH16ROM((address)) )
#define m68k_read_immediate_32(address) ( ( (address) & 0x800000) ? FETCH32RAM((address)) : FETCH32ROM((address)) )

/* Read data relative to the PC */
#define m68k_read_pcrelative_8(address)  ( FETCH8ROM((address)) )
#define m68k_read_pcrelative_16(address) ( FETCH16ROM((address)) )
#define m68k_read_pcrelative_32(address) ( FETCH32ROM((address)) )
...
INLINE uint m68ki_read_8(uint address)
{
  uint addr = ADDRESS_68K(address);
  if (addr < 0x800000) return FETCH8ROM(addr);
  if (addr >= 0xFF0000) return FETCH8RAM(addr);
...
{
  uint addr = ADDRESS_68K(address);
  if (addr < 0x800000) return FETCH16ROM(addr);
  if (addr >= 0xFF0000) return FETCH16RAM(addr);
...
{
  uint addr = ADDRESS_68K(address);
  if (addr < 0x800000) return FETCH32ROM(addr);
  if (addr >= 0xFF0000) return FETCH32RAM(addr);
